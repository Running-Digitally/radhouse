...
that revision. A private stream configured with
`automatic_private_release: true` instead hands Builder's exact preview to
Reviewer, then sends a matching READY result to Deployer without per-release
owner messages. Reviewer findings still return to Builder. It requires
...
review, merge and deployment continue to require owner messages unless that
project channel explicitly opts into automatic private release.
